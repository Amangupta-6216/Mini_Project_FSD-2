...
export default function BooksPage() {
  ...
  const handleDelete = async (id, title) => {
    ...
    try {
      ...
      toast.error('Cannot delete - may have active transactions')
    }
  }
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <div className="page-title">{isAdmin ? 'Book Catalog' : 'Browse Books'}</div>
      ...
      <div className="toolbar">
        <form className="search-box" onSubmit={handleSearch} style={{ maxWidth: 400 }}>
          ...
          <input id="book-search" placeholder="Search by title, author, genre, ISBN..." value={search} onChange={e => setSearch(e.target.value)} />
        </form>
      ...
      <div className="table-wrapper">
        <table className="data-table">
          <thead><tr>
            ...
            <th>Year</th><th>Copies</th><th>Available</th>{isAdmin && <th>Actions</th>}
          </tr></thead>
          <tbody>
            {books.length === 0 && (
              <tr><td colSpan={isAdmin ? 9 : 8}><div className="empty-state"><div className="empty-icon">BK</div><p>No books found</p></div></td></tr>
            )}
            {books.map((b, i) => (
              <tr key={b.id}>
                ...
                {isAdmin && (
                  <td>
                    <div style={{ display: 'flex', gap: 6 }}>
                      ...
                      <button className="btn btn-sm" style={{ background: 'var(--danger-light)', color: 'var(--danger)' }} onClick={() => handleDelete(b.id, b.title)}>Delete</button>
                    </div>
                  </td>
                )}
              </tr>
