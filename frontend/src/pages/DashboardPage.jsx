...
export default function DashboardPage() {
  ...
  const cards = stats ? [
    { label: 'Total Books', value: stats.totalBooks, icon: 'BK', type: 'accent' },
    { label: 'Total Members', value: stats.totalMembers, icon: 'MB', type: 'info' },
    { label: 'Books Issued', value: stats.issuedBooks, icon: 'IS', type: 'warning' },
    { label: 'Overdue', value: stats.overdueBooks, icon: 'OD', type: 'danger' },
    { label: 'Available', value: stats.availableBooks, icon: 'AV', type: 'success' },
  ] : []
              ...
              <tbody>
                {overdue.map(t => (
                  <tr key={t.id}>
                    ...
                    <td style={{ color: 'var(--text-secondary)' }}>{t.member?.name}</td>
                    <td style={{ color: 'var(--danger)', fontWeight: 600 }}>Rs. {t.fine?.toFixed(0)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
