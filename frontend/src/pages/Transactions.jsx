...
export default function Transactions() {
  ...
  const openIssue = () => {
    ...
    getMembers().then(r => setMembers(r.data.filter(m => m.status === 'ACTIVE')))
  ...
  const handleIssue = async (e) => {
    e.preventDefault()
    try {
      await issueBook(Number(form.bookId), Number(form.memberId))
  ...
  const handleReturn = async (id, title) => {
    ...
    try {
      ...
      if (data.fine > 0) toast.success(`Returned! Fine: ₹${data.fine.toFixed(0)}`, { duration: 5000 })
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <div className="page-subtitle">{transactions.length} total records</div>
        </div>
        <button className="btn btn-primary" id="issue-book-btn" onClick={openIssue}>📤 Issue Book</button>
      </div>

      <div className="toolbar">
        {['all','issued','overdue','returned'].map(t => (
      ...
      <div className="table-wrapper">
        <table className="data-table">
          <thead><tr>
            <th>#</th><th>Book</th><th>Member</th><th>Issue Date</th>
            <th>Due Date</th><th>Return Date</th><th>Status</th><th>Fine (₹)</th><th>Action</th>
          </tr></thead>
          <tbody>
            {displayed.length === 0 && (
              <tr><td colSpan={9}><div className="empty-state"><div className="empty-icon">🔄</div><p>No records</p></div></td></tr>
            )}
            {displayed.map((t, i) => (
              <tr key={t.id}>
          ...
          <div className="modal">
            ...
            <form onSubmit={handleIssue}>
              <div className="modal-body">
                ...
                <div className="form-group">
                  <label className="form-label">Select Member (Active only) *</label>
                ...
                <div style={{ padding: '10px 14px', background: 'var(--info-light)', borderRadius: 8, fontSize: 13, color: 'var(--info)' }}>
                  ℹ️ Loan period: <strong>14 days</strong>. Fine: <strong>₹5/day</strong> after due date.
                </div>
              </div>
