...
export default function Members() {
  ...
  const handleSave = async (e) => {
    e.preventDefault()
    try {
      if (editing) { await updateMember(editing, form); toast.success('Member updated!') }
      else { await createMember(form); toast.success('Member added!') }
      setModal(false); load()
    } catch(err) { toast.error(err.response?.data?.error || 'Failed to save member') }
  }

  const handleDelete = async (id, name) => {
    if (!window.confirm(`Remove "${name}" from the system?`)) return
    try { await deleteMember(id); toast.success('Member removed'); load() }
    catch { toast.error('Cannot delete member with active transactions') }
  }
    ...
    <div>
      ...
      <div className="table-wrapper">
        <table className="data-table">
          ...
          <tbody>
            {members.length === 0 && (
              <tr><td colSpan={7}><div className="empty-state"><div className="empty-icon">👥</div><p>No members found</p></div></td></tr>
            )}
            {members.map((m, i) => (
              <tr key={m.id}>
                ...
                <td>
                  <div style={{ display: 'flex', gap: 6 }}>
                    ...
                    <button className="btn btn-sm" style={{ background: 'var(--danger-light)', color: 'var(--danger)' }} onClick={() => handleDelete(m.id, m.name)}>🗑️</button>
                  </div>
                </td>
              </tr>
            ))}
